...
Для запуска тестов и получения отчета о покрытии кода, убедитесь что у вас установлен `pytest-cov`:
pip install pytest-cov

Затем выполните:
pytest

Флаги для генерации отчета о покрытии уже добавлены в `pytest.ini`. Отчет будет выведен в консоль, а также будет создан HTML-отчет в директории `htmlcov/`. Откройте `htmlcov/index.html` в браузере для детального просмотра.
