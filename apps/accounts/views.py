        ...
        try:
            ...
            if referral_code:
                try:
                    ...
                    logger.info(
                        f"User {email} registered with referral code {referral_code}")
                except User.DoesNotExist:
                    logger.warning(
                        f"Invalid referral code {referral_code} used during registration")

            password = generate_password()
            logger.info(f"Generated password for user {email}")
            ...
            if referral_code and user.referral:
                ...
                messages.success(
                    self.request, 'Регистрация прошла успешно. Данные для входа отправлены на ваш email.')
            return super().form_valid(form)
        except Exception as e:
            logger.exception(
                f"Unexpected error during registration with email {email}")
            messages.error(self.request,
...
class ForgotPasswordView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(
                self.request, 'Новый пароль был сгенерирован и отправлен на ваш email.')
            return super().form_valid(form)
        except User.DoesNotExist:
            logger.exception(
                f"User with email {email} not found during password reset")
            messages.error(
                self.request, 'Пользователь с таким email не найден.')
            return self.form_invalid(form)
        except Exception as e:
            logger.exception(
                f"Unexpected error during password reset with email {email}")
            messages.error(self.request,
                           'Произошла непредвиденная ошибка при восстановлении пароля. Пожалуйста, попробуйте позже.')
            return self.form_invalid(form)
...
def dashboard(request):
    user = request.user
    additional_emails = user.additional_emails.split(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                logger.info(
                    f"Added additional email {email} for user {user.username}")

                return JsonResponse({'success': True, 'emails': emails})
            except Exception as e:
                logger.exception(
                    f"Error adding email {email} for user {user.username}")
    ...
    if request.method == 'POST':
        ...
        if email in emails:
            try:
                ...
                user.save(update_fields=['additional_emails'])
                logger.info(
                    f"Removed additional email {email} for user {user.username}")
                return JsonResponse({'success': True, 'emails': emails})
            except Exception as e:
                logger.exception(
                    f"Error removing email {email} for user {user.username}")
...
def update_balance(request):
    if request.method == 'POST':
        try:
            ...
            logger.info(
                f"Balance updated for user {user.username}: -{cost} = {user.balance}")

            return JsonResponse({'success': True, 'new_balance': user.balance})
        except (ValueError, TypeError):
            logger.exception(f"Invalid value provided for update_balance")
            return JsonResponse({'success': False, 'message': 'Неверное значение.'}, status=400)
        except Exception as e:
            logger.exception(
                f"Error updating balance for user {request.user.username}")
            return JsonResponse({'success': False, 'message': 'Произошла ошибка при обновлении баланса.'}, status=500)
