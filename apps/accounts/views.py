...
from vagvin.monitoring import record_user_login, record_user_registration
...
class LoginView(FormView):
    ...
    def form_valid(self, form: LoginForm) -> HttpResponse:
        ...
        if user is not None:
            ...
            record_user_login()  # Record login metric
            return redirect("accounts:dashboard")
...
class RegisterView(FormView):
    ...
    def form_valid(self, form: RegistrationForm) -> HttpResponse:
        ...
        if success:
            record_user_registration()  # Record registration metric
...
class UnifiedCheckView(LoginRequiredMixin, View):
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> JsonResponse:
        ...
        try:
            ...
            logger.info(
                f"User {request.user.username} requested unified check for VIN: {vin}"
            )
            ...
            autoteka_result = AutotekaService.check(
                vin, "vin"
            )  # Assume VIN check for Autoteka here
            ...
            try:
                ...
                logger.info(
                    f"Saved unified query for user {request.user.username}, VIN {vin}"
                )
            except Exception:
                # Log failure but don't block returning results to user
                logger.exception(
                    f"Failed to save unified query history for user {request.user.username}, VIN {vin}"
                )
            ...
            logger.exception(
                f"Unexpected error during unified check for user {request.user.username}"
            )
            return JsonResponse(
                {"error": "Внутренняя ошибка сервера при проверке"}, status=500
            )
