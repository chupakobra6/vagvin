...
class UserService:
    ...
    @transaction.atomic
    def register_user(
        cls, email: str, referral_code: Optional[str] = None
    ...
    def _process_referral(cls, user: User, referral_code: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"User {user.email} registered with referral code {referral_code}"
            )
    ...
    def reset_password(cls, email: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if (
                last_reset
                and (now - last_reset).total_seconds() < settings.PASSWORD_RESET_TIMEOUT
            ):
                ...
                return (
                    False,
                    f"Пароль был недавно сброшен. Подождите ещё {wait_time} минут, прежде чем запрашивать снова.",
                )
            ...
            if not email_sent:
                ...
                return (
                    False,
                    "Не удалось отправить письмо. Попробуйте позже или обратитесь в службу поддержки.",
                )
            ...
            return (
                True,
                "Новый пароль был создан и отправлен на ваш адрес электронной почты",
            )
    ...
    def get_additional_emails(cls, user: User) -> List[str]:
        ...
        return [
            email.strip()
            for email in user.additional_emails.split(",")
            if email.strip()
        ]
    ...
    def add_additional_email(
        ...
        match (email, emails):
            ...
            case (_, lst) if len(lst) >= 5:
                return (
                    False,
                    "Вы не можете добавить более 5 дополнительных email-адресов",
                    emails,
                )
            case _:
                pass
