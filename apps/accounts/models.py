...
class User(AbstractUser, BaseModel):
    ...
    overdraft = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Овердрафт"
    )
    ...
    referral_code = models.CharField(
        max_length=32, unique=True, verbose_name="Реферальный код"
    )
    last_password_reset = models.DateTimeField(
        null=True, blank=True, verbose_name="Последний сброс пароля"
    )
    additional_emails = models.TextField(
        blank=True, verbose_name="Дополнительные email (CSV)"
    )
