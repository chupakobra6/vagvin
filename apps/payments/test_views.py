...
class TestPaymentViews:
    ...
    def test_initiate_robokassa_payment(self, mock_create_payment_with_url, client, user):
        """Test initiating a Robokassa payment."""
        payment = Payment(id=1, user=user, amount=Decimal("50.00"))
        mock_create_payment_with_url.return_value = (payment, "http://test-payment-url.com")
    ...
    def test_robokassa_callback_valid(self, mock_verify, client, user):
        ...
        payment = Payment.objects.create(id=1, user=user, amount=100, total_amount=110, provider='robokassa', invoice_id='inv1')
    ...
    def test_yookassa_callback_invalid(self, mock_verify, client, user):
        ...
        payment = Payment.objects.create(id=2, user=user, amount=100, total_amount=110, provider='yookassa', invoice_id='inv2')
