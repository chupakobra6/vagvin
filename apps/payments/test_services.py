...
class TestPaymentService:
    ...
    def test_process_payment_success(self, user):
        ...
        initial_balance = user.balance
        success, data = PaymentService.process_payment(
            user, Decimal("50.00"), "Test Purchase"
        )
        ...
        assert (
            Payment.objects.filter(
                user=user, provider="internal", status="success"
            ).count()
            == 1
        )
    ...
    def test_process_payment_insufficient_funds(self, user):
        ...
        initial_balance = user.balance
        success, data = PaymentService.process_payment(
            user, Decimal("2000.00"), "Expensive Purchase"
        )
