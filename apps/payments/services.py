...
from vagvin.monitoring import record_payment
...
class PaymentProcessor:
    ...
    def create_payment(
        self, user, amount: Decimal, total_amount: Optional[Decimal] = None
    ) -> Payment:
...
class TestModePaymentProcessor(PaymentProcessor):
    ...
    def __init__(self, real_processor: PaymentProcessor):
        ...
        super().__init__(
            f"test_{real_processor.provider_name}", real_processor.commission_rate
        )
    ...
    def mark_payment_successful(self, payment: Payment) -> bool:
        ...
        if success:
            logger.info(
                f"Test payment {payment.id} marked as successful and balance updated"
            )
        else:
    ...
    def verify_callback(self, params: Dict[str, Any]) -> Tuple[Optional[Payment], bool]:
        ...
        try:
            payment = Payment.objects.get(
                invoice_id=invoice_id, provider=self.provider_name, status="pending"
            )
        except Payment.DoesNotExist:
            return None, False
...
class YookassaPaymentProcessor(PaymentProcessor):
    ...
    def create_payment_url(self, payment: Payment, user) -> str:
        ...
        payload = {
            "amount": {"value": str(payment.total_amount), "currency": "RUB"},
            "confirmation": {
                "type": "redirect",
    ...
    def verify_callback(self, data: Dict[str, Any]) -> Tuple[Optional[Payment], bool]:
        ...
        idempotence_key = (
            data.get("object", {}).get("metadata", {}).get("idempotence_key", "")
        )
...
class HeleketPaymentProcessor(PaymentProcessor):
    ...
    def verify_callback(self, params: Dict[str, Any]) -> Tuple[Optional[Payment], bool]:
        ...
        try:
            payment = Payment.objects.get(
                invoice_id=order_id, provider=self.provider_name, status="pending"
            )
        except Payment.DoesNotExist:
            return None, False
...
class PaymentService:
    ...
    def mark_as_successful(payment) -> None:
        ...
        record_payment("success")  # Record successful payment metric
    ...
    def mark_as_failed(payment) -> None:
        ...
        payment.save(update_fields=["status", "updated_at"])
        record_payment("failed")  # Record failed payment metric
    ...
    def update_balance(cls, user, amount: Decimal) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Balance {log_action} for user {user.username}: {abs(amount)} = {user.balance}"
            )

            return True, {"new_balance": user.balance, "amount": amount}
    ...
    def can_afford(cls, user, amount: Decimal) -> Tuple[bool, str]:
        ...
        if available >= amount:
            ...
            return (
                False,
                f"Недостаточно средств. Вам не хватает {needed} руб. для совершения платежа.",
            )

    @classmethod
    @transaction.atomic
    def process_payment(
        cls, user, amount: Decimal, description: str
    ...
    def get_user_payments_stats(cls, user) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_provider": {
                item["provider"]: {"count": item["count"], "total": item["total"]}
