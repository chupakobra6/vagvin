...
from vagvin.monitoring import record_report_generation
    ...
    def _process_request(self, data) -> JsonResponse:
        ...
        if data.get("vin"):
            ...
            if avito_id:
                ...
                return JsonResponse(
                    {
                        "error": "Невозможно извлечь ID объявления из предоставленной ссылки Avito"
                    },
                    status=400,
                )
        else:
            ...
            return JsonResponse(
                {
                    "error": "Необходимо указать VIN, регистрационный номер или ссылку на Avito"
                },
                status=400,
            )

        result = AutotekaService.check(input_value, input_type)
        record_report_generation()  # Record report generation metric
...
class CarfaxCheckView(View):
    ...
    def _process_request(self, data) -> JsonResponse:
        ...
        if not vin:
            ...
            return JsonResponse(
                {"error": "Необходимо указать VIN автомобиля"}, status=400
            )
    ...
    try:
        ...
        if website_user:
            ...
            Query.objects.create(
                user=website_user, vin=vin.upper(), query_type=query_type_value
            )
