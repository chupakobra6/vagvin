...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--approved", action="store_true", help="Generate only approved reviews"
        )
    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(f"Generating {count} test reviews...")
        )
        ...
        for _ in range(count):
            ...
            admin_response = (
                random.choice(admin_responses)
                if is_approved and random.random() < 0.6
                else None
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully generated {reviews_created} test reviews."
            )
        )
        return None
